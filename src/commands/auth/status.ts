...
function runStatus(): void {
  ...
  if (mode === 'oauth') {
    ...
    if (tokens?.expiry_date) {
      ...
      logger.info(`Expiry:      ${expiry.toISOString()}${isExpired ? ' (expired — will refresh on next use)' : ''}`);
    }
    if (tokens?.scope) {
    ...
    if (!credPath) {
      logger.warn('No credentials configured. Run `gacli auth login` or set a service account via:\n  gacli config set credentials /path/to/service-account.json');
    } else {
